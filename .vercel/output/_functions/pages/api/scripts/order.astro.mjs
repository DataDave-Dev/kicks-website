...
const POST = async ({ request, cookies }) => {
  try {
    ...
    const { data: mov, error: movError } = await supabase.from("Consecutivos").select("Consecutivo").eq("Tipo", 1).single();
    console.log(mov);
    if (movError) {
    ...
    const hasInvalidItems = cartItems.some(
      (item) => !item.Producto || !item.Cantidad || !item.Precio
    );
    if (hasInvalidItems) {
    ...
    const orderDetails = cartItems.map((item) => ({
      ...
      Impuestos: (item.Cantidad * item.Precio * 0.16).toFixed(2),
      Total: (item.Cantidad * item.Precio * 1.16).toFixed(2)
    }));
    const { error: orderDetailError } = await supabase.from("PedidosDetalles").insert(orderDetails);
    if (orderDetailError) {
    ...
    const { error: updateConsecError } = await supabase.from("Consecutivos").update({ Consecutivo: nextConsecutivo }).eq("Tipo", 1);
